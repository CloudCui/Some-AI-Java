...
public class Gbfs {
		...
		public void search(){
			...
			System.out.println(file + " GBFS " + numberOfNodes);
		...
		/**
		 * Sort the fringe 
		 * in the order of small h(n) to large h(n)
		 */
		public void sort(){
			...
			for(int i = 0; i < fringe.size(); i++){
				for(int j = 1; j < fringe.size() - i; j++){
					...
					if(fringe.get(i).getTotalHn() > fringe.get(i + j).getTotalHn()){
