...
/**
 * This is the class which the program uses to execute the uniform-cost search
 * 
 * @author Xiangyun
 *
 */
...
public class Ucs {
		...
		public void search(){
			...
			while(!solutionFound){
				...
				//sort the fringe, with the one has least g(n) comes first
				sort();
			...
			System.out.println(file + " UCS " + numberOfNodes);
		...
		/**
		 * Sort the fringe
		 * in the order of small g(n) to large g(n)
		 */
		public void sort(){
			...
			for(int i = 0; i < fringe.size(); i++){
				for(int j = 1; j < fringe.size() - i; j++){
					...
					if(fringe.get(i).getTotalGn() > fringe.get(i + j).getTotalGn()){
